...
export default function SearchButton({
    ...
    function handleSubmit(e: FormEvent<HTMLFormElement>) {
        e.preventDefault();
        router.push(`/specifications/${p}?u=${u}&r=${r}&s=${searchTerm}`);
    }

    return (
        <form action="submit" className="flex flex-row justify-end items-center" onSubmit={handleSubmit}>
            ...
            <Link href={`/specifications/${p}?u=${u}&r=${r}&s=${searchTerm}`} className="btn btn-primary ml-4">Search</Link>
        </form>
    )
}